...
def _terminal_status(state: LearnLoopState) -> str | None:
    """Shared terminal check for macro_router and finalize_status.

    Returns the terminal status, or None when the loop should continue.
    """
    settings = get_settings()
    macro = state.get("macro_iter", 0)
...
def finalize_status(state: LearnLoopState) -> dict:
    ...
    if state.get("status") == "failed":
        return {"phase": "done"}
    # macro_router only routes here when a terminal condition holds, so this is
    # non-None in practice; fall back to max_iter_reached defensively.
    return {"status": _terminal_status(state) or "max_iter_reached", "phase": "done"}
